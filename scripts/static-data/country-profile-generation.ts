...
async function loadCountryProfileInputs(rootDir: string, includeFinalCountryTierInput: boolean) {
  ...
  const statsRows = buildCountryStats(rules, supportedAdmin2Keys, seeds, dataset.cities, admin1ByCountry, admin2ByCountry, dataset.countries);
...
export async function generateCountryTierCandidateReport(rootDir = process.cwd()): Promise<CountryTierCandidateReport> {
  ...
  const { rules, overrideSeed, seeds, dataset, supportedAdmin2Keys, statsRows } = await loadCountryProfileInputs(rootDir, false);
