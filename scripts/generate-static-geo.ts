...
const publicCountryGeoDir = path.join(publicGeoDir, 'countries');
...
async function admin1FeaturesForCountry(
  ...
  if (geoBoundariesAdm1) {
    const matched = matchedGeoBoundariesFeatures(countryCode, geoBoundariesAdm1, admin1Items, (item) => `admin1:${countryCode}.${(item as GeoNamesAdmin1).admin1Code}`);
...
async function admin2FeaturesForCountry(
  ...
  const matched = matchedGeoBoundariesFeatures(countryCode, geoBoundaries, admin2Items, (item) => {
...
function assertRequiredGeoFeatures(report: GeoBoundaryReport, packagesByOutputPath: Map<string, GeoJsonFeature[]>, citiesPayload: CitiesPayloadWire): void {
  ...
  const worldFeatures = packagesByOutputPath.get('apps/web/public/data/geo/world.geojson') ?? [];
  const worldKeys = featureRegionKeys(worldFeatures);
  const outlineKeys = featureRegionKeys(packagesByOutputPath.get('apps/web/public/data/geo/region-outlines.geojson') ?? []);
  const viewExpectations = viewRegionKeyExpectations(citiesPayload);
  ...
  for (const country of report.countries) {
    ...
    const countryPath = `apps/web/public/data/geo/countries/${country.countryCode}.geojson`;
...
await rm(publicCountryGeoDir, { recursive: true, force: true });
...
for (const countryCode of [...detailedCountryCodes].sort()) {
  ...
  const generated = await admin1FeaturesForCountry(countryCode, admin1Items, admin2Items, dataset.cities, naturalEarthAdmin1Low, naturalEarthAdmin1, geoBoundarySourceSeed, iso3ByCountry);
...
const report: GeoBoundaryReport = {
  ...
  packages: [
    await writeTrackedGeoPackage('apps/web/public/data/geo/world.geojson', worldFeatures, 1),
    await writeTrackedGeoPackage('apps/web/public/data/geo/region-outlines.geojson', regionOutlineFeatures, 1)
  ],
  countries: []
};
...
for (const countryCode of [...c3CountryCodes].sort()) {
  ...
  const admin1Generated = admin1GeneratedByCountry.get(countryCode) ?? await admin1FeaturesForCountry(countryCode, admin1Items, allAdmin2Items, dataset.cities, naturalEarthAdmin1Low, naturalEarthAdmin1, geoBoundarySourceSeed, iso3ByCountry);
  ...
  report.packages.push(await writeTrackedGeoPackage(`apps/web/public/data/geo/countries/${countryCode}.geojson`, [
...
for (const countryCode of [...detailedCountryCodes].filter((countryCode) => !c3CountryCodes.has(countryCode)).sort()) {
  ...
  const admin1Items = admin1ByCountry.get(countryCode) ?? [];
  const admin2Items = allAdmin2ByCountry.get(countryCode) ?? [];
  const admin1Generated = admin1GeneratedByCountry.get(countryCode) ?? await admin1FeaturesForCountry(countryCode, admin1Items, admin2Items, dataset.cities, naturalEarthAdmin1Low, naturalEarthAdmin1, geoBoundarySourceSeed, iso3ByCountry);
  countryReports.push({
    countryCode,
