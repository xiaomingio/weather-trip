...
describe('map region weather summaries', () => {
  it('groups global daily weather by country', () => {
    const summaries = buildDailyRegionSummaries(
      [
  ...
  it('groups detailed country travel weather by GeoNames admin1 code', () => {
    ...
    const summaries = buildTravelRegionSummaries([california, nevada], forecastsByCity, { ...filter, region: 'country:US' }, 'en');
  ...
  it('keeps China province summaries keyed by display adcode', () => {
    const yunnan = city({
      id: 'kunming',
      country: 'China',
      countryCode: 'CN',
    ...
    const summaries = buildDailyRegionSummaries([yunnan], [forecast({ cityId: yunnan.id })], '2026-07-21', 'country:CN', 'zh');
