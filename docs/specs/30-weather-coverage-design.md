...
两档体验都使用范围内全部城市。城市规模按 3,500+ 设计，公开数据包控制在 5,000 个以内；列表、marker 和聚合可以承受。地图 marker 密度交互见 `docs/specs/41-weather-map-interactions.md`。

全球/大洲视图中的一级区域叠加层主要用于概览，不把用户直接带到某个省州筛选。用户可以从地图或下拉进入 C2/C3 国家详情，再在国家工作区选择一级区域；C3 二级区域只用于国家详情图的着色、hover 和覆盖复核。
...
| 候选类型 | 例子 | 设计判断 |
| --- | --- | --- |
| C3 候选 | CN、ES、FR、IT、PE、CH | 二级区域数量可控，或二级区域语义接近旅行天气区域，国家页下钻有明显价值 |
| C2 候选 | US、RU、BR、CA、MX、JP、IN、ID、AR、CL | 面积、纬度/经度跨度、海拔差异或旅行需求强，但二级区域数量过多或语义过细 |
...
`data/generated/country-tier-candidate-report.md` 由 `scripts/generate-country-tier-candidates.ts` 根据 GeoNames、旅游目的地输入、覆盖规则和 admin2 输入生成。每行是一个候选国家，按 C3 候选优先级排序，包含：

| 维度 | 表中字段 | 用途 |
| --- | --- | --- |
| 候选优先级 | `国家`、`中文名`、`C3 候选`、`C3 分`、`C2 候选`、`C2 分` | 判断是否值得进入人工复核队列 |
| 城市成本 | `C1 点数`、`C2 点数`、`C3 点数`、新增点数、重合点数 | 估算升 C2/C3 后会增加多少天气城市 |
| 空间差异 | `面积 km2`、`纬度跨度`、`经度跨度`、`海拔跨度 m`、`空间分` | 判断国家级颜色是否会抹平内部天气差异 |
| 旅行需求 | `旅行分/数量` | 判断用户是否有进入国家页比较区域天气的需求 |
| 行政粒度 | `admin1`、`支持的 admin2/admin2`、`admin2 覆盖率` | 判断一级/二级区域数量是否可控，二级区域是否有足够城市代表点 |
| 数据缺口 | `缺 admin1 代表点`、`缺 admin2 代表点` | 判断升档后是否需要补点、映射或放弃 |

候选报告只输出候选命中、分数和指标，不输出 C2/C3 理由。C3 是比 C2 更细的国家层级，命中 C3 候选的国家也会自动进入 C2 复核队列。人工判断写在 `country-tier-countries.yml` 的 `reason` 备注里，后续生成产物只用 `countryCode` 和 `countryTier`；其中 C1 行不会产生详细区域覆盖。
...
C2 候选国家通常包括 US、RU、BR、CA、MX、JP、IN、ID、AR、CL、CO、EG、MA、TH、TZ、ZA 等。它们的共同特征是面积或空间跨度大、旅行需求明显，二级区域数量过多或语义过细，按一级区域已经能表达主要旅行天气差异。候选国家需要先看 `country-tier-candidate-report.md` 的优先级、旅行价值、城市增量和边界成本，再写入 `country-tier-countries.yml`。
...
C3 候选国家通常包括 CN、ES、FR、IT、PE、CH 等。中国这类地区需要确认大陆二级区域是否接近地级旅行区域，并把港澳台 companion 区块纳入详情图；其他候选国家需要看面积、旅游热度、二级区域数量、城市覆盖率和边界成本。实际城市会和一级区域、人口兜底、旅游目的地城市重合一部分。

C3 国家需要重点检查二级区域的语义质量，避免把不稳定的二级区域直接变成国家页默认体验。
...
二级区域代表点也是最低覆盖要求。中国大陆西部、秘鲁安第斯、智利南北跨度区域、瑞士和奥地利阿尔卑斯区域这类天气差异强的地方，可以保留多个旅游代表城市。
...
中国大陆按地级行政区（介于省和县之间的城市或地区层级）选择代表城市，范围包括地级市、地区、自治州、盟，并把北京、上海、天津、重庆四个直辖市作为必选代表点。香港、澳门和台湾在中国详情图中按 companion C3 区块聚合，仍保留为城市字典和全球视图里的独立 C1 地区。

这个口径参考中国大陆真实行政区划层级。代表点优先使用人工确认的旅游目的地，其次看行政代表性和人口。

中国详情图里的二级区域是地级市、地区、自治州、盟、自治区直辖县级行政区以及港澳台 companion 区块，不等于页面上的城市记录。城市用于刷新天气、展示 marker 和做区域聚合。
...
例如中国、美国、加拿大、澳大利亚、俄罗斯这类 C2/C3 候选地区/国家，在全球/大洲按一级区域着色可以避免一个大色块抹平内部差异。

例如中国、西班牙、法国、意大利、秘鲁、瑞士这类 C3 候选地区/国家，进入详情页后可以考虑显示更细的二级区域色块。

一个区域没有样本时不用指标色，不用相邻区域、国家平均或几何中心天气推断。地图保留白色半透明区块和边界；区域 tooltip 显示当前语言的区块名和“暂无数据 / No data”。区域 tooltip 和图例必须让用户知道颜色来自样本聚合。

区域 tooltip 至少展示样本数、代表城市和聚合方式。样本数为 1 的区域要明确显示代表城市名称，避免用户把区域颜色理解成整个行政区的面积平均天气。
...
全球或大洲下，子地区控件不作为筛选项展示。主地区下拉只列全球/大洲和 C2/C3 国家；C1 国家只有少量代表点，不进入地区下拉。用户可以从地图或下拉进入详细国家，再在国家工作区选择一级区域。


| 当前视图  | 点击对象                   | 行为                                   |
| --------- | -------------------------- | -------------------------------------- |
| 全球/大洲 | C1 国家                    | 保持当前地区或显示 tooltip，不进入细分 |
| 全球/大洲 | C2 / C3 国家或其一级区域面 | 进入该国家详情                         |
| C2 国家   | 一级区域面                 | 进入该一级区域                         |
| C3 国家   | 二级区域面                 | 显示二级区域 hover，不改变地区筛选     |
| 任意视图  | 城市 marker                | 选中城市 forecast，不改变当前地区      |

### 视野与视觉

切换地区时，地图优先使用目标区域的边界计算 bounds；只有没有边界或没有区域 summary 时，才退回城市 bounds。
...
区域面只需要有统一区域标识和 geometry；中英文名由地点/区域字典提供；运行时逻辑不按国家或地区做特殊分支。
...
1. 对二级区域数量大的 C2 候选国家，后续可以做“进入一级区域后再懒加载二级区域”，不在全国详情一次性展开
2. 给 C3 候选国家补人工确认的重点旅游目的地，避免只靠行政中心和人口
