...
城市库生成完成后会再计算公开默认排序，也就是前端解码得到的 `rank`。这个排序服务 Weather Map 默认列表和低 zoom marker 避让；它不复用入选优先级。默认排序先按 GeoNames 行政级别，再按人口：国家首都 `PPLC` 排在最前，首都之间按人口从高到低；之后依次是一级行政中心、二级/三级/四级行政中心、普通人口城市，最后用入选优先级、国家 code 和稳定 ID 兜底。
...
Weather Map 和 City Finder 使用同一套城市到天气区块的归属规则。运行时先按当前地区筛选城市，再把每个城市映射到 `country:*`、`admin1:*` 或 `admin2:*` 天气聚合 key，最后用这些 key 匹配 MVT feature 的 `weatherRegionKey`，没有 `weatherRegionKey` 时匹配 `regionKey`。City Finder 不按当前结果列表、搜索关键词或可见 marker 重新给区块归属；筛选条件只改变该城市给所属区块贡献的匹配天数和指标值。

| 工具 | 区块归属 | 指标贡献 |
| --- | --- | --- |
| Weather Map | 与 City Finder 相同，按当前 region 和国家 C1/C2/C3 档位把城市归入天气区块 | 选中日期的单日 forecast，`comfort` 使用单日舒适度平均 |
| City Finder | 与 Weather Map 相同，按当前 region 和国家 C1/C2/C3 档位把城市归入天气区块 | 当前 `days` 窗口内的 forecast，`comfort` 使用匹配天数 / 总天数 |
...
| wind          | 样本平均最大风速                             |
| elevation     | 样本城市平均海拔                             |
| comfort       | Weather Map 使用单日舒适度平均；City Finder 使用匹配天数 / 总天数 |
